...
- [x] Интегрировать Firebase Storage


Задача: Поддержка подписок iOS и премиум-доступа на вебе
Описание:
Реализовать механизм предоставления премиум-доступа в веб-приложении на основе подписок, купленных в iOS-приложении.
Требования:

Сделать сервис, который будет принимать от АЙОС уведомления, что какой-то юзер полу
В терминологии Apple это S2S-уведомления — Server-to-Server notifications.
То есть сервис на backend, который принимает уведомления от Apple о событиях подписки:
Покупка (DID_RENEW / INITIAL_BUY) → isPaid = true
Отмена (CANCEL) → isPaid = false
Возврат (REFUND) → isPaid = false
Неудачное продление (DID_FAIL_TO_RENEW) → isPaid = false
В нашей задаче он будет реализован как endpoint /apple/notifications на backend, который:
Получает POST от Apple.
Валидирует уведомление (подпись, JWT, receipt и т.д.).
Обновляет поле isPaid пользователя в базе.
(Опционально) Логирует событие для аудита.
То есть кратко: это Apple S2S Notifications Handler.
Если хочешь, я могу набросать пример такого сервиса на NestJS, готового к интеграции с нашим IUser и подписками. Хочешь, чтобы я это сделал?

Авторизация в iOS
Пользователь должен залогиниться через Google / Firebase / Email / Apple ID.
Все покупки и restore привязываются к этому аккаунту (firebaseUid).
Покупка и Restore Purchase
Новые покупки → receipt → backend → verify у Apple → isPaid = true.
Старые покупки → Restore Purchase → receipt → backend → verify → обновление isPaid.
Backend как источник правды
Все данные о подписке хранятся в базе (apple_purchases).
Поле isPaid у пользователя отражает актуальный премиум-статус.
S2S уведомления Apple (опционально)
Backend endpoint /apple/notifications получает события отмены, возврата или неудачного продления подписки.
Статус подписки обновляется мгновенно (isPaid = false при отмене/возврате).
Веб-приложение
Пользователь логинится → backend проверяет isPaid → отображает премиум-функции.
Restore в вебе не нужен, ключевой момент — общий аккаунт между приложением и вебом.
Цель:
Обеспечить безопасную и актуальную синхронизацию премиум-статуса между iOS-приложением и веб-приложением без использования сторонних сервисов, таких как RevenueCat.


Задача: Настроить загрузку файлов для неавторизованных пользователей (гостей) с ограничениями.
Требования:
Для гостя использовать фиктивный UID: guest.
Применить лимиты Free-пользователя:
Максимальный размер файла по типу (PDF/JPG/PNG и др.)
Максимальное количество файлов в день
Максимальное количество файлов за один запрос
Генерировать короткоживущий signed URL (1–2 часа) для скачивания файлов гостем.
Авторизованные пользователи остаются без изменений (длительный URL, свои лимиты).
Отдавать на фронт JSON с:
{
...
Проверка формата и размера файла должна работать одинаково для гостей и авторизованных.
Цель: гость может конвертить/загружать файлы без регистрации, но с ограничениями Free-пользователя.
...
### Backend:
- [х] Создать модуль `users`: `nest g module users`
- [х] Создать сущность User для Prisma
- [х] Создать сервис для управления пользователями
- [] Эндпоинты: `/users/profile`, `/users/stats`

## 🗃️ ШАГ 5: Настройка БД и Prisma
- [х] Настроить Prisma конфигурацию в `backend`
- [x] Создать миграции для User и File сущностей
- [x] Проверить связь между таблицами
- [ ] Настроить репозитории

 План на 05.02.26
- Обновить FilesService и FilesController:
При загрузке файла в Firebase сразу создавать запись в таблице File
Записывать userId, filename, storageKey, status='uploaded'
Возвращать пользователю ссылку на файл и его id
